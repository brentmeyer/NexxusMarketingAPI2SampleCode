...
function ValidateForm()
{
	...
	if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
		$emailErr = 'Email address is invalid or empty.';
		$validForm = false;
	}
...
function UpsertResource($clientObject, $parameters)
{
	$resultArray = Array();
	$resultArray['id'] = 0;
	$resultArray['success'] = false;
	...
	try {
		$result = $clientObject->Upsert($parameters);
	}
	catch (Exception $e) {
		error_log( "Nexxus Marketing Registration sample: SOAP call Upsert failed - ".$e->getMessage() );
		throw($e);
	}
	if ($result->UpsertResult->BatchCompleted){
		foreach ($result->UpsertResult->Resources as $resultResource) // Should be only 1
		{
			if ($resultResource->OperationSucceeded == true){
				$fieldarray=(array)$resultResource->Resource->Field;
				$key=array_search("Id", $fieldarray);
...
function UpsertAnalyticsMapping($client,$contactId)
{

	$webSiteVisitorId=$_COOKIE["__aavo"];
	if ($webSiteVisitorId == null){
		error_log( "Nexxus Marketing Registration sample: Couldn't read tracking cookie (__aavo) for contact $contactId");
	  throw new Exception("An internal error occurred. However, your contact details and preferences were successfully recorded.");
	}

	//Build contact field opbjects.
	$entityNameObject= (object)array("_" => "Contact", "Id"=>"EntityName");
	$entityIdObject = (object)array("_" => $contactId, "Id"=>"EntityId");
	$dataSourceObject = (object)array("_" => "WebAnalytics", "Id"=>"DataSource");
	$webSiteVisitorObject = (object)array("_" => $webSiteVisitorId, "Id"=>"WebsiteVisitorId");


	// Parameter structure
	$fields = array($entityNameObject,
			$entityIdObject,
			$dataSourceObject,
			$webSiteVisitorObject);

	$resource = array("Field" => $fields);
	$resources = array("Resource" => $resource);
	$parameters = array("TypeId"=>"AnalyticsMapping",
			"IndexId"=>"DataSource_EntityName_WebsiteVisitorId",
			"Resources" => $resources);

	// create the AnalyticsMapping resource
	try{
		$result = UpsertResource($client,$parameters);
	}
	catch(Exeption $e){
		error_log( "Nexxus Marketing Registration sample: Couldn't map contact $contactId to WebsiteVisitorId $WebsiteVisitorId - ".$e->getMessage());
		 throw new Exception("An internal error occurred. However, your contact details and preferences were successfully recorded.");
	}
	return $result;
...
*/
	...
	catch (Exception $e)
	{
		...
		throw new Exception("Couldn't reach the remote server");
	}
  return $client;
}
...
// Only process POST requests.
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		try{
			...
			echo "There was an issue with your registration - ".$e->getMessage();
			return false;
  		}
		// record analytics mapping and activity. Not essential for recording contact details.
		try{
      UpsertAnalyticsMapping($client,$result["id"]);
